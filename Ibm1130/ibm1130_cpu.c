...
MTAB cpu_mod[] = {{UNIT_MSIZE, 4096, NULL, "4KW", &cpu_set_size},
...
DEVICE cpu_dev = {
    "CPU", &cpu_unit, cpu_reg, cpu_mod,  1,          16,   16,         1,
    16,    16,        &cpu_ex, &cpu_dep, &cpu_reset, NULL, cpu_attach, NULL}; /* attaching to CPU creates cpu log file */
...
static void
weirdop(const char *msg)
{
    printf("Weird opcode: %s at %04x\n", msg, IAR - 1);
}

static const char *xio_devs[] = {"dev-00?",  "console", "1442card", "1134ptape", "dsk0",     "1627plot", "1132print",
                                 "switches", "1231omr", "2501card", "sca",       "dev-0b?",  "sys7",     "dev-0d?",
                                 "dev-0e?",  "dev-0f?", "dev-10?",  "dsk1",      "dsk2",     "dsk3",     "dsk4",
                                 "1403prt",  "dsk5",    "2311drv2", "dev-18?",   "2250disp", "2741term", "dev-1b",
                                 "dev-1c?",  "dev-1d?", "dev-1e?",  "dev-1f?"};

static const char *xio_funcs[] = {"func0?", "write", "read", "sense_irq", "control", "initw", "initr", "sense"};
static int32
ibm1130_qcount()
{
    int32   i, cnt;
...
sim_instr(void)
{
    ...
    while (reason == 0) {
        ...
        if (F && F_bit_used[OP]) { /* long instruction, except for a few that don't have a long mode, like WAIT */
            ...
            /* to do: the previous steps may lead to incorrect GUI SAR/SBR display if the instruction doesn't actually fetch
             * anything. Check this. */
        ...
        case 0x01:                        /* --- XIO --- */
            ...
            if ((cpu_unit.flags & (UNIT_ATT | UNIT_TRACE_IO)) == (UNIT_ATT | UNIT_TRACE_IO))
                trace_io("* XIO %s %s mod %02x addr %04x", xio_funcs[iocc_func],
                         (iocc_func == XIO_SENSE_IRQ) ? "-" : xio_devs[iocc_dev], iocc_mod, iocc_addr);
        ...
        case 0x02: /* --- SLA,SLT,SLC,SLCA,NOP - Shift Left family --- */
            ...
            case 0x0040: /* SLCA */
                if (TAG) {
                    ...
                    C = (CCC != 0);
                    WriteIndex(TAG, (ReadIndex(TAG) & 0xFF00) |
                                        CCC); /* put low 6 bits back into index register and zero bits 8 and 9 */
                    break;
                }
        ...
        case 0x0e:   /* --- MDX - Modify Index and Skip --- */
            if (F) { /* long format: adjust memory location */
                if (TAG) {
                    ...
                    oldval = ReadW(word2);
                    DSPLC =
                        IR & 0x00FF; /* use extended displacement (no INDIR bit, it's is part of displacement in this op) */
                    if (DSPLC & 0x0080)
                        DSPLC |= ~0xFF;
...
static int
simh_status_to_stopcode(int status)
{
    return (status == SCPE_BREAK) ? STOP_BREAK
                                  : (status == SCPE_STOP) ? STOP_IMMEDIATE : (status == SCPE_STEP) ? STOP_STEP : STOP_OTHER;
}
...
static t_bool
bsctest(int32 DSPLC, t_bool reset_V)
{
...
void
break_simulation(t_stat stopreason)
{
    reason = stopreason;
}
...
t_stat
cpu_set_size(UNIT *uptr, int32 value, CONST char *cptr, void *desc)
{
    t_bool used;
    int32  i;
...
t_stat
cpu_set_type(UNIT *uptr, int32 value, CONST char *cptr, void *desc)
{
    REG *r;
    ...
    for (r = cpu_reg; r->name != NULL; r++) { /* unhide or hide 1800-specific registers & state */
        if (simh_strnicmp(r->name, "XR", 2) == 0) {
            if (value & UNIT_1800)
...
void
xio_1131_switches(int32 addr, int32 func, int32 modify)
{
    char msg[80];
...
t_stat
register_cmd(const char *name, t_stat (*action)(int32 flag, CONST char *ptr), int arg, const char *help)
{
    int i;
...
static void
archive_backtrace(const char *inst)
{
    static int prevind;
...
void
void_backtrace(int afrom, int ato)
{
    int i;
...
static t_stat
backtrace_cmd(int32 flag, CONST char *cptr)
{
    int n;
...
static t_stat
cpu_attach(UNIT *uptr, CONST char *cptr)
{
...
trace_instruction(void)
{
    ...
    if (new_log) {
        ...
        fprintf(cpu_unit.fileref, " IAR%s  ACC  EXT %s XR1  XR2  XR3 CVI %sOPERATION" CRLF, syms ? "           " : "",
                log_fac ? " (flt)   " : "", log_fac ? "     FAC      " : "");
    ...
    fprintf(cpu_unit.fileref, "%04x %04x %s%04x %04x %04x %c%c%c %s", ACC & 0xFFFF, EXT & 0xFFFF, fltstr, M[1] & 0xFFFF,
            M[2] & 0xFFFF, M[3] & 0xFFFF, C ? 'C' : ' ', V ? 'V' : ' ', (ipl < 0) ? ' ' : (ipl + '0'), facstr);
...
static void
trace_common(FILE *fout)
{
    fprintf(fout, "[IAR %04x IPL %c] ", IAR, (ipl < 0) ? ' ' : ('0' + ipl));
}

void
trace_io(const char *fmt, ...)
{
    va_list args;
...
static t_stat
view_cmd(int32 flag, CONST char *cptr)
{
#ifdef _WIN32
    char                cmdline[256];
    STARTUPINFO         sinfo = {0};
    ...
    CreateProcess(NULL, cmdline, NULL, NULL, TRUE, NORMAL_PRIORITY_CLASS, NULL, NULL, &sinfo, &pinfo);
    /* Don't really care about the startup or process information. */
    if (sinfo.hStdInput)
...
void (*cgi_start_hook)(void) =
    NULL; /* these can be defined by a CGI wrapper to do things on start and stop of simulation */
